...
\begin{flushright}
Chris Miller  %% Fill in your name here
...
\begin{eqnarray*}
	\begin{bmatrix}
	w_0 x_{11}^2 + w_1 x_{11}x_{12} \\[0.3em]
    w_0 x_{11}x_{12} + w_1 x_{12}^2
	\end{bmatrix} +
    \begin{bmatrix}
	w_0 x_{21}^2 + w_1 x_{21}x_{22} \\[0.3em]
    w_0 x_{21}x_{22} + w_1 x_{22}^2
	\end{bmatrix} +
    \dots +
    \begin{bmatrix}
	w_0 x_{N1}^2 + w_1 x_{N1}x_{N2} \\[0.3em]
    w_0 x_{N1}x_{N2} + w_1 x_{N2}^2
	\end{bmatrix} =
    \begin{bmatrix}
    \sum_{n} (w_0 x_{n1}^2 + w_1 x_{n1}x_{n2}) \\[0.3em]
    \sum_{n} (w_0 x_{n1}x_{n2} + w_1 x_{n2}^2)
    \end{bmatrix}
\end{eqnarray*}

Step 4: solve $\mathbf{X}^\top\mathbf{X} \mathbf{w}$

From Problem 3 we know that $\mathbf{X}^\top\mathbf{X}$ = 
    $\begin{bmatrix}
    \sum_{n=1}^N x_{n1}^2 & \sum_{n=1}^N x_{n1}x_{n2} \\[0.3em]
    \sum_{n=1}^N x_{n1}x_{n2} & \sum_{n=1}^N x_{n2}^2
    \end{bmatrix}$
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{randomtxt.png}
\caption{\label{fig:random}Confirmation that the random.txt file could be opened in a text editor.}
\end{figure}
...
\begin{verbatim}
    ...
    for x in range(10):
        roll2dice = np.random.randint(1, 7, (1000, 2))
        
        double6 = np.array([6, 6])
        count = 0
        
        for row in roll2dice:
...
first, the 1000 by 2 array was made containing values from 1 to 6 inclusive. Then each row was checked using a for loop to see if the row values both equaled 6. If that was true, then a counter was incremented. The result was that out of 1000 rolls of two 6-sided dice, double sixes were rolled 26. 

2.

Running 9 more times resulted in: 27, 21, 26, 21, 31, 27, 45, 26, 21. The original result (26) appeared two more times. 

3. 

After resetting the seed back to 8, the results matched the original 10 results. 

4. 

It is important to have controlled random number sequences so that we can have consistency when we are testing or debugging our program. 
