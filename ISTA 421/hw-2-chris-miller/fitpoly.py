...
def fitpoly(x, t, model_order):
    ...
    Lambda = 0
    identityMatrix = numpy.identity(X.shape[1])
    identityMatrix[0][0] = 0
    
    #w = numpy.linalg.lstsq(X.T.dot(X) + Lambda * identityMatrix, X.T.dot(t))[0] # verification
    w = numpy.dot(numpy.linalg.inv(X.T.dot(X) + Lambda * identityMatrix), X.T.dot(t))
...
def read_data_fit_plot(data_path, model_order=1, scale_p=False,
    ...
    A "top-level" script to
        ...
        (7) Optionally call the matplotlib show() fn, which keeps the plot open
...
def fit_data(filename, model_order=1, scale_p=False, plot_p=False):
    """
    This function is the same as the fit_mens100_data function, but it loads 
    any data file instead of the men's 100 meter data file.

    :filename: String containing the name of the csv file containing the data
    :param model_order: Integer representing the polynomial model order
    :param scale_p: Boolean specifying whether to scale the input data between [0, 1] before fitting
    :param plot_p: Boolean specifying whether to
    :return:
...
#fit_data('womens100.csv', model_order=1, scale_p=True, plot_p=True)
fit_data('synthdata2018.csv', model_order=7, scale_p=True, plot_p=True)
