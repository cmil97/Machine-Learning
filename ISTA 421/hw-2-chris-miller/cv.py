...
def run_cv( K, maxorder, x, t, testx, testt, randomize_data=True, title='CV' ):
    """
    ...
    for p in range(maxorder + 1):
        ...
        for fold in range(K):
            ...
            Z = X[:, 0:(p+1)]
            Lambda = 0
            identityMatrix = numpy.identity(Z.shape[1])
            identityMatrix[0][0] = 0

            w = numpy.dot(numpy.linalg.inv(Z.T.dot(Z) + Lambda * identityMatrix), Z.T.dot(t))
    ...
    plot_cv_results(log_mean_train_loss, log_mean_cv_loss, None,
...
def problem4():
    """
    This function loads synth 2018 data, then runs 5-fold cross validation
    and leave one out cross validation, then plots the error.
    """
    ...
    # hold the place for run_cv function
    w = numpy.array([0, 1, 5, 2])
    xmin = -6
    xmax = 6
    sigma = 50
    ...
    # run the cross-validtion program
    run_cv( K, 7, x, t, testx, testt, randomize_data=True, title='{0}-fold CV'.format(K) )
    
    # run again for LOOCV (K = 25 in this case)
    K = 25
    run_cv( K, 7, x, t, testx, testt, randomize_data=True, title='LOOCV' )
    
    # plot data
    # plot_data(x, t)
    
    # get the best model parameters (w) and then make the plot
    #plot_model(x, w)
