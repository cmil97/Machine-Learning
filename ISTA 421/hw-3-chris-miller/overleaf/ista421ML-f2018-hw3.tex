...
\begin{align*}
    \mathbf{E}_{p(x)} \left\{ 35 + 3x - 0.5x^3 + 0.05x^4 \right\}  = \int_{-4}^{10} \! (35 + 3x - 0.5x^3 + 0.05x^4)(\frac{1}{14}) \, \mathrm{d}x. =
\end{align*} 

\begin{align*}
    \frac{1}{14} [35x + 1.5x^2 - 0.125x^4 + 0.01x^5] |_{-4}^{10}
    = 29.16
\end{align*}

\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{approx_expected_value.png}
\caption{\label{fig:1}Results for problem 2.}
\end{figure}
...
\caption{\label{fig:6}Plot of 20 functions for polynomial orders 5 and 9}
\end{figure}
...
\caption{\label{fig:8}New plot of 20 functions for polynomial orders 5 and 9}
\end{figure}
...
The Fisher Information can be computed using the following formula:

\begin{align*}
    I =
    \mathbf{E}_{Bern(x)} \left\{ -\frac{\partial^2log(x|p)}{\partial^2p} \right\}
\end{align*}
